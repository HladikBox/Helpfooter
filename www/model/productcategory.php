...
public function _list($search_param,$orderby){
    ...
    if(isset($search_param["productlist"]))
    {
        $sql_where.=" and r_main.productlist like '%".$search_param["productlist"]."%'";
    }
    $sql="select  r_main.id  ,r_main.seq ,r_main.name,r_main.markcode ,r_main.icon,r_main.summary ,r_main.content ,r_main.status ,r_main.productlist  from  tb_productcategory r_main  where 1=1 $sql_where  and r_main.status<>'D' 
    $orderby";
