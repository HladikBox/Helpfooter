...
public function _list($search_param,$orderby){
    ...
    $sql="select  r_main.id  ,r_main.seq ,r_main.markcode ,r_main.name ,r_main.summary ,r_main.content ,r_main.status  from  tb_service r_main  where 1=1 $sql_where  and r_main.status<>'D' 
    $orderby";
...
public function get($id){
    ...
    $sql="select  r_main.id  ,r_main.seq ,r_main.markcode ,r_main.name ,r_main.summary ,r_main.content ,r_main.status  from  tb_service r_main  where r_main.id=$id ";
                $query = $this->dbmgr->query($sql);
